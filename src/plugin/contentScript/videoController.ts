...
export type SkippableVideoControls = {
  [key in keyof VideoControls]: (
    ...
    ...args: Parameters<VideoControls[key]>
  ) => ReturnType<VideoControls[key]>
}

export const setupVideoEventHandlers = (
  port: Port,
  ...
  const skipEvents = [...playLike, ...pauseLike, ...seekLike].reduce(
...
export const setNewVideoTimeIfNecessary = (
  ...
  if (
    ...
    (force ||
      Math.abs(video.currentTime - newVideoTime) >
        acceptableTimeDifferenceBetweenClientsInSeconds)
  ) {
...
export const skipEvents = (
  skipEvents: SkipEvents,
  ...eventNames: Array<keyof SkipEvents>
): void => {
