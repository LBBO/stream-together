<template>
  <form @submit="() => {}">
    <fieldset name="general">
      ...
      <input id="backend-url" type="url" v-model="backendURL" />
    </fieldset>
  </form>
  <small v-if="!backendUrlIsValid" class="error">
    Couldn't save settings because some values are invalid!
  </small>
  <small v-else-if="isSaving"> Saving settings </small>
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const setIsSavingIfHasntChangedSince = (
      newValue: boolean,
      date = new Date(),
    ) => {
      const now = new Date()
    ...
    const backendUrlIsValid = computed(() =>
      validateBackendUrl(backendURL.value),
    )
